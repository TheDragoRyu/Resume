...
- Split the featured-flag update into `planFeaturedFlagUpdates` and `commitFeaturedFlagUpdates`. `saveProjectConfiguration` resolves every destination and computes every new file body before `.featured-repos.local.json` is rewritten, and restores already-written Markdown if a later write fails.
